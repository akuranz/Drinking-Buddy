...
function renderCocktail() {
  ...
  $.ajax({
    ...
    while (n < 15) {
      ...
      ingredients.push(
        drink[`strIngredient${n + 1}`] + ": " + drink[`strMeasure${n + 1}`]
      );
    ...
    $("#ingredients").html(
      ingredients
        .map(ingredient => `<li class="cocktail">${ingredient}</li>`)
        .join("")
    );
    $("#directions").html(directions);
    $("#idk").html(
      `<button class="idk btn" type="submit">Give Me a Different One!</button>`
    );
